...
import { createScanLogin } from '../lib/scan-login';
...
const scan = createScanLogin({
  ...
  scanPageUrl: import.meta.env.VITE_SCAN_PAGE_URL || `http://192.168.31.93:5173/scan${import.meta.env.DEV ? '?env=local' : ''}`,
...
const canRefresh = computed(() => ['EXPIRED', 'CANCELLED', 'CONSUMED', 'ERROR'].includes(state.value.phase));
  ...
  <div class="wechat-login" :aria-busy="busy">
    <div class="qr-image-area" :class="{ 'is-masked': masked }">
      ...
      <div v-if="masked" class="qr-overlay" role="status" aria-live="polite" aria-atomic="true">
        <p class="qr-overlay-title">{{ title }}</p>

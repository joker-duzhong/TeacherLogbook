...
onBeforeUnmount(() => { disposed = true; revision++; if (confirming) ElMessageBox.close(); });
...
async function save() {
  ...
  try {
    ...
    await auth.api.saveRecord(selectedClass,resource,values,editId.value||undefined);if(!active())return;editing.value=false;ElMessage.success('已保存');
    if(['students','committee-roles'].includes(resource))await workspace.reloadReferences();await load();
  } catch(cause){if(active()){if(cause instanceof ApiError)errors.value={...cause.fields};ElMessage.error(workspace.message(cause));}} finally{saving.value=false;}
}
async function remove(row: BusinessRecord) {
  if(!active()||busy.value||saving.value)return;
  saving.value=true;confirming=true;
  try{
    await ElMessageBox.confirm('删除后该记录将不再显示，确认删除？','删除记录',{type:'warning'});
...
async function toggleTodo(row: BusinessRecord) { if(!active()||busy.value||saving.value)return;saving.value=true;try{await auth.api.saveRecord(selectedClass,resource,{status:row.status==='已完成'?'待完成':'已完成'},row.id);await load();}catch(cause){if(active())ElMessage.error(workspace.message(cause));}finally{saving.value=false;} }
...
async function search(){if(saving.value)return;if(dateFrom.value&&dateTo.value&&dateFrom.value>dateTo.value){error.value='开始日期不能晚于结束日期';return;}pageNumber.value=1;await load();}
...
<template>
  <div class="business-toolbar">
    ...
    <el-button :loading="busy" @click="search">查询</el-button><el-button :disabled="busy||saving" @click="exportRows">导出全部结果</el-button><el-button type="primary" :disabled="busy||saving" @click="openForm()">新增</el-button>
  </div>
  ...
  <el-table v-else v-loading="busy" :data="displayRows" stripe empty-text="暂无记录" class="business-table">
    ...
    <el-table-column label="操作" fixed="right" :width="resource==='todos'?200:140"><template #default="scope"><el-button v-if="resource==='todos'" link :disabled="busy||saving" @click="toggleTodo(scope.row)">{{ scope.row.status==='已完成'?'重开':'完成' }}</el-button><el-button link type="primary" :disabled="busy||saving" @click="openForm(scope.row)">编辑</el-button><el-button link type="danger" :disabled="busy||saving" @click="remove(scope.row)">删除</el-button></template></el-table-column>
  </el-table>

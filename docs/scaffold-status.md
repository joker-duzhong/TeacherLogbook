...
- 后续网页登录态修订：扫码与手机号登录统一持久化 access token，刷新及重新打开先通过 `/auth/me` 验证；401 或退出清除缓存，不保存 refresh token、不自动续期。网络故障保留缓存但暂不开放工作台，允许手动重试。小程序内存会话不变，最新验证见 CHANGELOG。
- PC 和小程序已实现 7 组导航、39 个业务视图、19 类记录，以及班级、仪表盘、排座位、文件、备份与旧版迁移功能。
- MR、薄荷、ngrok、Apple 四套皮肤共享变量定义，偏好通过真实后端保存；不再使用登录后的占位工作台。
- 共享类型由实际 OpenAPI 的 67 条路径生成；业务更新使用后端模块内新增的 POST 兼容入口，保留原 PATCH。
- 前端 84 项单元测试、旧版 11 项测试、4 个工作区类型检查及双端构建通过；Playwright 6 组测试覆盖桌面全部页面、19 类新增表单、增改删、4 套皮肤、座位、文件、401，以及 390/320 窄屏布局。
- 后端业务模块 43 项隔离测试通过；修复真实数据统计、全量学生导出、筛选、座位交换、备份校验与回滚。未修改 common、core 或公共配置。
- 当前 PC 地址为 http://127.0.0.1:5174/login。小程序 AppID 未配置，真实账号、数据库并发及微信真机尚未完成验收；不将隔离测试视为线上全量通过。
- 详见 `full-migration-checklist.md` 的功能矩阵、检查命令和全部修改文件。下列内容保留为历史记录，不代表当前功能范围。

## 历史状态：PC 扫码接入完成，双端骨架构建通过
